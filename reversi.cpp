...
{
    if (posX <= (GROESSE_X - 1) && posY <= (GROESSE_Y-1) && posX >= 0 && posY >= 0)
    {
    	return true;
    }
    else
...
bool zugGueltig(const int spielfeld[GROESSE_Y][GROESSE_X], const int aktuellerSpieler, const int posX, const int posY)
{
	int gegner = 3 - aktuellerSpieler; // dasselbe wie: if aktuellerSpieler == 1 -> gegner = 2
	    ...
	    for (int j = -1; j <= 1; j++)
	    {
	        for (int i = -1; i <= 1; i++)
	        {
	        	while(aufSpielfeld(posY,posX) && aufSpielfeld(posY +j, posX+i) && spielfeld[posY +j][posX +i] == gegner)
	        	{
	        		for (int u = 2; u <= 7; u++)
	        		{
	        			if( aufSpielfeld(posY +u*j, posX+u*i) && spielfeld[posY +u*j][posX +u*i] == aktuellerSpieler)
	        			{
	        				return true;
	        			}
	        			else if ( aufSpielfeld(posY +u*j, posX+u*i) && spielfeld[posY +u*j][posX +u*i] == gegner)
	        			{
	        				continue;
	        			}
	        			else
        	...
        	while(aufSpielfeld(posY,posX) && aufSpielfeld(posY +j, posX+i) && spielfeld[posY +j][posX +i] == gegner)
        	{
        		for (int u = 2; u <= 7; u++)
        		{
        			if( aufSpielfeld(posY +u*j, posX+u*i) && spielfeld[posY +u*j][posX +u*i] == aktuellerSpieler)
        			{
        				for (int z = 1; z < u; z++)
        				{
        					spielfeld[posY + (u-z)*j][posX + (u-z)*i] = aktuellerSpieler;
        				}
...
int moeglicheZuege(const int spielfeld[GROESSE_Y][GROESSE_X], const int aktuellerSpieler)
{
	...
	for (int yY = 0; yY <GROESSE_Y; yY++)
	{
		for (int xX = 0; xX < GROESSE_X; xX++)
		{
		    while(spielfeld[yY][xX] == 0)
		    {
			    if (zugGueltig(spielfeld, aktuellerSpieler, xX, yY))
			    {
			    	a++;
			    }
			    break;
...
bool menschlicherZug(int spielfeld[GROESSE_Y][GROESSE_X], const int aktuellerSpieler)
{
    ...
    while (true)
    {
        ...
        else if (zugGueltig(spielfeld, aktuellerSpieler, posX, posY))
        {
            //accept turn;
            break;
        }
...
int main()
{
    ...
    int spielerEingabe;
    int spielerTyp[2]; // Feld, das Informationen ueber den Typ des Spielers enthaelt. MENSCH(=1) oder COMPUTER(=2)
    std::cout << "Geben Sie bitte der Zahl der menschlichen Spieler: ";
    ...
    case 1:
    	...
    	break;
    case 2:
    	spielerTyp[0] = MENSCHEINZ;
    	spielerTyp[1] = MENSCH;
    	break;
    default:
    	std::cout << "Falsche Eingabe, wird geschlossen.";
    	exit(1);
    }
    spielen(spielerTyp);
    //
